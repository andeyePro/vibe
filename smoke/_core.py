...
#
# Later hand-edits (each one intentional, each verified to change ONLY the
# named keys — never a regeneration):
#   - task_041: postStartCommand gains the `"${VIBE_EXTRA_DOMAINS:-}"` argv to
#     init-firewall.sh, and containerEnv gains VIBE_EXTRA_DOMAINS (containerEnv
#     rather than remoteEnv so `docker inspect` can see it and
#     extra_domains_drift can compare against it).
#   - task_043: remoteEnv gains GEMINI_API_KEY (the /review gemini slot's
#     key, carried exactly like ZOTERO_API_KEY — remoteEnv only, never
#     containerEnv, so the secret never reaches the firewall/postStart
#     layer). remoteEnv is the ONLY key touched.
# Phase 1a: hand-add three non-secret Claude delegation routing variables to
# remoteEnv; no other golden fields regenerated. The Codex login bind is NOT
# here: it is injected by _build_override_config only for opted-in projects.
TASK034_GOLDEN_DEVCONTAINER_RENDER_OP_OFF = '{\n  "name": "vibe",\n  "image": "vibe-dev:latest",\n  "runArgs": [\n    "--cap-add=NET_ADMIN",\n    "--cap-add=NET_RAW"\n  ],\n  "customizations": {\n    "vscode": {\n      "extensions": [\n        "anthropic.claude-code",\n        "dbaeumer.vscode-eslint",\n        "esbenp.prettier-vscode",\n        "eamodio.gitlens"\n      ],\n      "settings": {\n        "editor.formatOnSave": true,\n        "editor.defaultFormatter": "esbenp.prettier-vscode",\n        "editor.codeActionsOnSave": {\n          "source.fixAll.eslint": "explicit"\n        },\n        "terminal.integrated.defaultProfile.linux": "zsh",\n        "terminal.integrated.profiles.linux": {\n          "bash": {\n            "path": "bash",\n            "icon": "terminal-bash"\n          },\n          "zsh": {\n            "path": "zsh"\n          }\n        }\n      }\n    }\n  },\n  "remoteUser": "node",\n  "mounts": [\n    "source=vibe-bash-history,target=/commandhistory,type=volume",\n    "source=vibe-claude-config,target=/home/node/.claude,type=volume",\n    "source=${localEnv:HOME}/.ssh,target=/home/node/.ssh-host,type=bind,readonly",\n    "source=${localEnv:HOME}/.gitconfig,target=/home/node/.gitconfig-host,type=bind,readonly",\n    {\n      "source": "/host/brain2",\n      "target": "/brain2",\n      "type": "bind"\n    },\n    {\n      "source": "/host/zotero",\n      "target": "/zotero",\n      "type": "bind",\n      "readonly": true\n    }\n  ],\n  "containerEnv": {\n    "NODE_OPTIONS": "--max-old-space-size=4096",\n    "CLAUDE_CONFIG_DIR": "/home/node/.claude",\n    "POWERLEVEL9K_DISABLE_GITSTATUS": "true",\n    "VIBE_SSH_AUTO": "${localEnv:VIBE_SSH_AUTO}",\n    "VIBE_EXTRA_DOMAINS": "${localEnv:VIBE_EXTRA_DOMAINS}",\n    "GIT_OPTIONAL_LOCKS": "0"\n  },\n  "remoteEnv": {\n    "GITHUB_TOKEN": "${localEnv:GITHUB_TOKEN}",\n    "VIBE_PROJECT_NAME": "${localEnv:VIBE_PROJECT_NAME}",\n    "GITHUB_REPO_SLUG": "${localEnv:GITHUB_REPO_SLUG}",\n    "ZOTERO_API_KEY": "${localEnv:ZOTERO_API_KEY}",\n    "GEMINI_API_KEY": "${localEnv:GEMINI_API_KEY}",\n    "VIBE_CLAUDE_P_BILLING": "${localEnv:VIBE_CLAUDE_P_BILLING}",\n    "VIBE_CLAUDE_P_SETTINGS": "${localEnv:VIBE_CLAUDE_P_SETTINGS}",\n    "VIBE_CLAUDE_P_CONFIG_DIR": "${localEnv:VIBE_CLAUDE_P_CONFIG_DIR}",\n    "OPENPROJECT_MCP_URL": "${localEnv:OPENPROJECT_MCP_URL}",\n    "OPENPROJECT_MCP_BEARER": "${localEnv:OPENPROJECT_MCP_BEARER}",\n    "OPENPROJECT_MCP_FWD_PORT": "${localEnv:OPENPROJECT_MCP_FWD_PORT}",\n    "NODE_OPTIONS": "--max-old-space-size=4096",\n    "CLAUDE_CONFIG_DIR": "/home/node/.claude",\n    "POWERLEVEL9K_DISABLE_GITSTATUS": "true",\n    "VIBE_SSH_AUTO": "${localEnv:VIBE_SSH_AUTO}",\n    "GIT_OPTIONAL_LOCKS": "0"\n  },\n  "workspaceMount": "source=${localWorkspaceFolder},target=/workspace,type=bind,consistency=delegated",\n  "workspaceFolder": "/workspace",\n  "postStartCommand": "printf %s \\"${GITHUB_TOKEN:-}\\" | sudo /usr/local/bin/init-firewall.sh \\"${VIBE_EXTRA_DOMAINS:-}\\" && sudo /usr/sbin/avahi-daemon --daemonize --no-drop-root 2>/dev/null || true && /usr/local/bin/setup-ssh.sh && /usr/local/bin/setup-git.sh && /usr/local/bin/write-env-hint.sh && /usr/local/bin/install-claude-extras.sh && /usr/local/bin/register-op-mcp.sh",\n  "waitFor": "postStartCommand"\n}\n'
