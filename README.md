...
- `vibe --agent <claude|codex>` — choose the lead runtime for this launch (default `claude`; see Codex-led sessions below)
...
The login mount is **opt-in per project and off by default**, and there is deliberately no machine-wide switch that turns it on. Interactive onboarding performs the following setup after host confirmation. For manual setup, all three prerequisites remain required:

1. **Log in**, so `~/.codex` exists — vibe never creates that directory itself.
2. **Create `.vibe-allow-codex`** in the project folder only if absent; refuse an existing symlink or non-regular file. The marker must be **untracked** (interactive onboarding adds local Git excludes before the initial commit): a committed marker, or one in a non-git download, is refused, exactly like `.vibe-allow-op`.
...
**Skills in a Codex-led container.** `$vs`, `$vss` and `$vsss` are Codex skills baked into the image at `/etc/codex/skills/{vs,vss,vsss}/SKILL.md` — the Admin scope root, so a project cannot add, shadow or disable one, and nothing is ever written into your Mac's `~/.codex` or `~/.agents` to make this work. Each SKILL.md tells Codex to read the single-sourced command body at `/usr/local/share/vibe/commands/<name>.md` and follow it verbatim, substituting a fixed table of Codex equivalents for the handful of Claude-only primitives those bodies use (`Agent`, `Skill`, `ScheduleWakeup`, and so on). The one primitive with no Codex equivalent at all is subagent dispatch — Codex's own `multi_agent` is pinned off in this image (see above) — so `/vs`'s Planner/Spec Critic/Generator/Tester/Evaluator roles become `vibe-delegate role <role> --model <luna|terra|sol|astra> --cwd <workspace>`: one fresh `codex exec` or `claude -p` process per role, run under the same hooks and firewall as the lead, never resuming or sharing a session with it. This ships the plumbing and its offline tests only; the first live Codex-led `/vs` trial is a later, separate item (see **MANUAL-TESTS Test 55**). Typing a bare `/vs` in Codex's own terminal is rejected by the composer before submission, but a line that starts with one leading space (` /vs …`) passes through as plain text with the space trimmed, and a managed `UserPromptSubmit` hook then adds context pointing the model at the `$vs` skill with the rest of the line as arguments — model-mediated, not guaranteed, so `$vs …` remains the reliable form. Nothing is written to the Mac's `~/.codex` to make this work, and the earlier `/prompts:`-based plan is withdrawn: Codex's custom-prompts directory is not read by the shipped binary. `/vs --panel N` has no subagents to fan out to either, so the image also ships `/usr/local/bin/codex-panel`: `codex-panel run --n N < diff` starts N independent, tool-less `codex exec` reviewers — one process each, a private temp cwd each, the same diff and a per-reviewer nonce on stdin — and prints one JSON object of verdicts for the chair to adjudicate. Its `--verify` mode drops `--ephemeral` so each reviewer's rollout is kept and can be proved to hold only its own nonce, which costs N live calls and leaves the diff and the nonces in `~/.codex/sessions` until you delete them, so it is an opt-in proof rather than the default. The same `$vs`/`$vss`/`$vsss` spelling also works when Claude Code is the lead: a synced CLAUDE.md fragment (`devcontainer/claude-md/dollar-prefix.md`) tells it that a prompt whose first token is one of those means invoke the matching command with the rest of the prompt as its arguments, verbatim.

**The unattended supervisor.** A `$vsss` run is meant to outlive its own quota window, and Codex's CLI has no durable loop of its own, so the image ships one: `/usr/local/bin/codex-supervisor` (root-owned, like the guard chain). It speaks Codex's app-server protocol on stdio — newline-delimited JSON, not JSON-RPC 2.0 — starting ONE thread with `approvalPolicy: "never"` and `sandbox: "danger-full-access"` (the only mode that works here; the container has no user namespaces for Codex's own bubblewrap sandbox, and the real boundary is the container, firewall and hooks described above), then re-entering that same thread with the literal word `continue` until the run's exit report ends with `VSSS-EXIT: <reason>`. It runs the vendor binary as `codex app-server` with no `-c` flags and a four-variable environment, so the system requirements and managed hooks apply unaltered. The reason it uses the protocol rather than `codex exec` is that only the protocol carries `codexErrorInfo` and the rate-limit snapshot: a quota failure names itself and reports when the window resets, so the supervisor sleeps until then plus two minutes rather than guessing, backs off on transient failures, declines every approval request it is sent (nobody is there to answer), and writes its thread id and counters to `.vss/codex-supervisor.json` after every change — so a killed supervisor resumes the same thread instead of starting a new one. Ceilings on turns, resumes, quota waits, retries and wall time bound the whole thing, and `codex-supervisor status --state <file>` prints where a run got to. It is connected to the explicit `--codex-run` / `--codex-resume` launcher path; offline protocol tests remain distinct from live-container acceptance.

#### Codex-led sessions (`vibe --agent codex`)

`vibe --agent codex` launches the ordinary vibe container — same image, same firewall, same mounts, same single-repo PAT — but starts the Codex TUI instead of Claude Code. The choice can also be a per-project **untracked** `.vibe/agent` file containing one word (`claude` or `codex`), or `VIBE_AGENT` in `~/.vibe/config`; the flag beats the remembered host-side folder choice, which beats the file and then the config var. A fresh interactive folder offers a choice; the unattended fallback is `claude`. Host memory contains only the canonical folder and runtime, never login material. Inside either agent, ask to switch or run `vibe-agent claude|codex`, then exit normally; Vibe reopens with that runtime after cleanup. Autonomous runs must be finished/stopped first. The request cannot grant login access and vendor conversations stay separate. A tracked, symlinked or malformed `.vibe/agent` is ignored with one `⚠` line and the next source is used — a committed one would let a pull request choose the lead runtime of every container that clones the repo.

Three prerequisites, all of them the Codex login mount's existing gates: `~/.codex` must exist on the Mac (from `codex -c 'cli_auth_credentials_store="file"' login`), the project must carry the untracked `.vibe-allow-codex` marker, and the project must be listed in `~/.vibe/codex-allow` (`vibe codex allow`). An interactive host launch guides missing setup with explicit consent; a noninteractive launch missing a prerequisite is refused **before** Docker does anything, naming both switches — Codex without its login is not a degraded session, it is a session that cannot work.

What is enforced once it starts: the launcher hands off to `/usr/local/bin/codex-entry`, a root-owned script that runs `codex-guard-liveness` first and refuses to start Codex at all unless the whole chain proves itself — every file root-owned and not group-writable, the requirements constraints active, every managed hook in its hardened form, four known-bad calls actually denied through the real adapter, a benign one still allowed, and the `codex --version` floor met. Only then does it `exec` the unmodified `codex` binary, with no model, approval or sandbox flag of its own: those belong to the root-owned policy in `/etc/codex`, which is the only place they can be audited. A refusal prints the failing check and exits; nothing is started.

**Claude Code remains the default lead.** Codex supports an explicit supervised path: `vibe --codex-run <project-local-prompt-file>` and `vibe --codex-resume <same-file>`. Plain `vibe --agent codex` stays interactive. The supervisor owns Codex continuation; Claude's separate auto-resume loop is not reused. See [Codex development setup](docs/codex-development.md) for context discovery, FM2C, OpenAI-only role defaults, recovery and rollout checks. Source and fixture readiness do not establish live Mac or container verification.
