...
4. **Autonomous runs**: the per-action SSH ask still applies. `touch .vibe-allow-ssh` in the project (or `VIBE_SSH_AUTO=1` host-side) to pre-authorise — then **relaunch**: the marker is read at container start, not per-turn, so a mid-session `touch` does nothing. **Keep the marker untracked** — a committed `.vibe-allow-ssh` grants autonomous SSH to every clone of the repo.
