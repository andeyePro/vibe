...
- **"Keep going / burn my session"** intent: suggest `/vsss` — it persists across credit-window resets BY DEFAULT until the task completes (relaunching via the auto-resume marker); mention `--sessions X` only as a CAP (total windows, e.g. `--sessions 3` = current window plus at most 2 relaunches) or `--sessions 1` for a single-window run.
