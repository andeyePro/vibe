...
- To record something: use the `/md` skill, or write the update directly into the relevant note (immediate - visible to every local agent + Obsidian at once; lands `authored`/grey, provisional until Martin authorises). `inbox/` is NOT a general knowledge tray - it holds ONLY the Zotero/OP stockpile queue for firewalled containers (`/brain2/inbox/zotero-queue.<source>.md`, `/brain2/inbox/op-queue.<source>.md`), which the gardener sweeps.
