...
- **Route by task class, not role prestige.** Fable 5's edge is long-horizon complex work; on small scoped calls Opus is near-parity and credits buy nothing.
- **Subscription models by default.** Credit-billed models run only with explicit per-task user consent (`CLAUDE.md § Non-goals`).

### Role defaults

| Role | Default | Why |
| --- | --- | --- |
| Planner + Evaluator | session model (Opus chair; `vibe --fable` only for huge/ambiguous tasks) | judgment interleaved with admin — mid-tier chair, expensive consultants |
...
| Panellists (`--panel`) | sonnet ×N (default 3) | blind fan-out of the review role; deliberately re-tiered from the upstream all-Opus pattern — N Sonnets beat 1 Opus because the value is INDEPENDENCE, not depth; one Opus depth-probe is the chair's optional extra on correlated consensus |

### Model plan (lives in spec.md)

Step 2's spec includes a **Model plan**: starting tier and escalation ceiling per role — e.g. `Generator: sonnet, ceiling opus; Fable rung: not pre-authorised`. Approving the spec pre-authorises or withholds the credit-billed rung. **Absent an explicit statement, the Fable rung is NOT pre-authorised.** Start at the tier the difficulty demands — don't burn cycles proving Sonnet can't.
...
Tier bumps happen on the Step-7 fail path, and ONLY when the Evaluator diagnoses a **capability failure** — reasonable approach, execution fell short. Spec ambiguity, brittle tests, scope creep and wrong-approach failures do NOT escalate; they route to spec revision / plateau handling — a model bump can't fix a bad spec.

- **Generator**: sonnet → opus after 2 consecutive capability-fails → **Fable 5 on the locked spec**, only with user consent (§ Fable grant). That rung re-dispatches the *Generator*, not the chair: compact brief, fresh context.
...
- `/vs --fable-gen <prompt>` — force a Fable Generator start (§ Model economy).
- `/vs --fable-subagents <prompt>` (alias `--fable`) — pre-authorise the Fable rung without forcing it. Full semantics: § Model economy.
...
- **Model plan** — tier + ceiling per role, a difficulty rationale, and an explicit `Fable rung: pre-authorised / not pre-authorised` line. Honor `--gen` / `--fable-gen` / `--fable-subagents` per § Model economy.
...
Spawn `Agent(subagent_type: "general-purpose", model: "<Model plan tier>")` — `"sonnet"` unless the Model plan or ladder says otherwise; `"fable"` only when that rung is user-authorised:
...
Ported 2026-07-10 from the agent-review-panel pattern per `.vs/audits/harness-landscape-2026-07.md`, re-tiered to Sonnet and re-sized: upstream runs 4–6 panellists; default 3 here, odd for tie-avoidance.

Sequencing: in fuzzy mode the panel occupies 5b's slot (5b itself is skipped). In rigorous mode it dispatches in the same batch position — after the Generator, alongside or immediately after the Tester (5a); the Evaluator reads both. Verdict interaction in rigorous mode: a red test suite fails the cycle regardless of panel opinion; a green suite + panel dissent goes to chair adjudication — the panel can sink a green cycle, it can never rescue a red one.
...
Each panellist gets exactly what the 5b Reviewer gets — `.vs/spec.md`, the original user prompt (pasted by Planner), `.vs/cycle-<N>/diff.patch` — and NOT the Generator's report, the Tester's output, or any other verdict. Each writes `.vs/cycle-<N>/panel/reviewer-<k>.md` (k = 1..N) with the same three sections as 5b (per-criterion assessment / concerns / verdict `pass`|`revise`|`fail`), honouring the run's `--plain`/`--techy` and resolved verbosity. Panellists do NOT touch `tasks.json` — with N concurrent writers that's a race; the chair updates it once after adjudication (Step 6). Under `--TDD`: panellists do not check the trail; the Tester does (§ Step 5a).

**Running under Codex.** Panellists are separate processes, not subagents: `codex-panel run --n N < diff` starts N independent tool-less `codex exec` reviewers, each in a private temp cwd with only the diff and its own nonce on stdin. It prints one JSON object (per-reviewer verdict, summary, findings, usage, plus a tally); the chair still runs the correlated-agreement check below on that JSON, and an incomplete panel exits 1, never a pass. `--verify` is the live isolation proof — it drops `--ephemeral` so each reviewer's rollout persists and can be checked for its own nonce and no sibling's. It needs Martin's per-run yes and leaves those rollouts — diff and nonces — in `~/.codex/sessions`.
...
- **Correlated consensus** (all N agree AND concern sets + rationale texture are near-duplicates): treat the panel as ONE reviewer's worth of signal, not N× confidence: unanimous-and-identical usually means the diff's surface steered every panellist down the same path. Log `panel: correlated consensus — low-information` in `.vs/progress.md`. For a high-blast-radius diff (security-adjacent files, launcher process control, anything in `CLAUDE.md § Invariants` territory) the chair MAY dispatch ONE additional Opus panellist as a depth probe; otherwise proceed but weight the panel accordingly.
...
- **Pass:** append verdict block to `.vs/progress.md` (including the Generator tier that passed). Update `TODO.md` / `CHANGELOG.md` per convention. On the FINAL cycle of a multi-cycle task (and in any `vs: archive <task-id> state (complete...)` commit), the task's umbrella `TODO.md` Open entry MUST be closed in the same commit — CHANGELOG-only close-outs are how umbrella entries go stale (task_017 sat "in flight" for 8 days after its archive commit). Commit with `/vs cycle <N>: pass` (or `/vs --fuzzy cycle <N>: pass`). Report to user: cycle number, key changes, how each criterion was verified (or in fuzzy mode, Reviewer's rationale highlights), and the final tier if the ladder escalated.
...
**Why opt-in:** observability overhead. Subagent dispatches each return a `<usage>` block (`total_tokens`, `tool_uses`, `duration_ms`); the chair's own tokens are summed from its session transcript at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, as `/budget` does. Run `/budget` for the month-to-date rollup regardless of `--cost`.
...
```
Tiers stay on separate lines because per-token costs differ; aggregating hides that. Fable is money rather than quota — always show its $ estimate.
