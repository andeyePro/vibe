...
- Language profiles: `devcontainer/profiles/<name>/Dockerfile` — thin child images built `FROM` the base `vibe-dev:latest` (`python` shipped; `--profile <name>`, `.vibe/profile`, or `VIBE_PROFILE` select one). Toolchain only, never project deps; see README's "Language profiles" section.
- Shipped extras: `devcontainer/commands/` (/budget, /c, /diet, /feast, /learn, /repo, /review, /ask, /sp, /vs, /vss, /vsss, /wide, /narrow), `devcontainer/agents/` (code-reviewer, code-writer, security-review, shellcheck-fixer, software-architect), `devcontainer/hooks/` (check-numbering.sh, copy-last-block.sh — both opt-in via `~/.claude/settings.json`), and `devcontainer/claude-md/` fragments (web-research, ssh-discipline, learnings, auto-memory-scope, content-guard, vibe-cli, workspace-is-the-repo, …). All synced into the persistent `~/.claude/` volume by `install-claude-extras.sh` on every container start; user-authored files in the same dirs are left alone.
...
- **Never add a `docker volume rm`, a `--volumes` prune, or a `docker rm -v` to the launcher.** `vibe-claude-config` holds the Claude Pro/Max login; `vibe-bash-history` holds shell history. Losing them to a Docker reset once (2026-09-17) was enough. `smoke/checks_34_docker_hygiene.py` lints the whole launcher for this, and asserts it against the docker call log of every `vibe clean` mode — if you need a new reclaim path, it goes through `vibe clean` and it stays volume-free.
- A spawned child never inherits this process's real stdin: `smoke/_core.py`'s `run()`/`run_bytes()` pass `stdin=subprocess.DEVNULL` whenever no `input`/`input_bytes` is given, and every direct `subprocess.run`/`Popen`/`check_output`/`call` in `smoke/*.py` does the same by hand where it doesn't route through those wrappers — a background/CI run with no controlling TTY leaves stdin open-but-never-written, and a child that reads from it hangs forever instead of failing fast (task_035, triggered by a 55-minute hang inside a `vibe learn` test). Run `python3 smoke-test.py < /dev/null` for any background/detached invocation, so a hang is impossible even if a new call site forgets this rule.
